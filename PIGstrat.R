...
library(cowplot)
library(ggpubr)
setwd("/Volumes/archive/cancergeneticslab/ConorM/SigsCombinedAnalysis")
...
d$start<-d$POS
d$end<-d$POS
d<-subset(d, experiment_code=="PIG" | group == "0D5 resequenced")
d<-subset(d, !Sample %in% c("PIG_A1A12", "PIG_E1D12"))
samps<-unique(d$sampleplot)[c(10,5:9,1:4)]
d$sampleplot<-factor(d$sampleplot, levels=samps)
table(d$Sample)
histplots<-ggplot(subset(d,experiment_code=="PIG"), aes(AF))+geom_density()+facet_wrap(sampleplot~.)+
...
ggsave(plot=histplots, file="pIG_strat/AFdist.png",width=10,height=10)
...
depth$sampleplot<-fct_relevel(depth$sampleplot, levels(depth$sampleplot)[c(11,10,5:9,1:4)])
depth$group<-factor(depth$group)
depth$group<-fct_relevel(depth$group,levels(depth$group)[c(4,1,3,2)])
...
ggsave(plot=mutrateplot, file="pIG_strat/mutrateplot.png",width=10,height=10)
...
levels(mat$X1)
mat$X1<-fct_relevel(mat$X1, levels(mat$X1)[c(11,12,10,5:9,1:4,22,17:21,13:16)])
mat$X2<-fct_relevel(mat$X2, levels(mat$X2)[c(11,12,10,5:9,1:4,22,17:21,13:16)])
...
ggsave(plot=cosmat0, file="pIG_strat/cos_heatmap0.png",width=20,height=20)
cosmat0+theme(axis.text=element_text(size=10),axis.title = element_text(size=10), text = element_text(size=5))
...
ggsave(plot=nsigscosim, file="pIG_strat/nsigscosim.png",width=20,height=12)
...
cossim3<-ggplot(solstats3, aes(Total.Mutations, Cosine.Similarity, col=Sample.Names))+
  ...
  theme(axis.text=element_text(size =20), axis.text.x = element_text(vjust=0.5),
        ...
        legend.text = element_text(size=20),legend.title = element_text(size=22))+
  scale_fill_gradient(low="white",high="red",guide_legend(title="Cosine \nsimilarity"))+
  geom_text(data=subset(solstats3, Sample.Names=="subclonal_PIG_E2B5"), aes(label=Sample.Names),size=10,vjust=-1)+
...
ggsave(plot=cossim3, file="pIG_strat/cossim3.png",width=20,height=12)
...
ggsave(plot=sigprofplot, file="pIG_strat/sigprofplot.png",width=10,height=10)
sigprofplot
...
TCWplot_clonal<-ggplot(Mc_clonal, aes(sampleplot))+geom_bar(aes(fill=Context),position = "fill")+
  ...
  theme(plot.margin = unit(c(3,1,1,1), "lines"), axis.text=element_text(size =20), axis.text.x = element_text(vjust=0.5,hjust=1, angle=90),
        axis.title = element_text(size=25),
        legend.text = element_text(size=20),legend.title = element_text(size=22))+scale_fill_manual(values = c("blue","red"))
ggsave(plot=TCWplot_clonal, file="pIG_strat/TCWplot_clonal.png",width=10,height=5)
TCWplot_clonal
...
TCWplot_subclonal<-ggplot(Mc_subclonal, aes(sampleplot))+geom_bar(aes(fill=Context),position = "fill")+
  ...
  theme(plot.margin = unit(c(3,1,1,1), "lines"), axis.text=element_text(size =20), axis.text.x = element_text(vjust=0.5,hjust=1, angle=90),
        axis.title = element_text(size=25),
        legend.text = element_text(size=20),legend.title = element_text(size=22))+scale_fill_manual(values = c("blue","red"))
ggsave(plot=TCWplot_subclonal, file="pIG_strat/TCWplot_subclonal.png",width=10,height=5)
TCWplot_subclonal
...
TCWplot
ggsave(plot=TCWplot, file="pIG_strat/TCWplot.png",width=10,height=10)
