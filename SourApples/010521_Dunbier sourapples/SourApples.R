...
cyto_gate_draw(gs,
               ...
               channels = c("FSC-A","FSC-H"))
cyto_names(gs)
LDFMO<-cyto_extract(gs, parent = "Single Cells")[c(20)]
LDFMO
cyto_gate_edit(gs,
               parent = "Single Cells",
...
cyto_names(gs)
cyto_gatingTemplate_apply(gs)
cyto_gate_draw(gs[c(20,22)], parent = "Live cells",
               alias = c("mCherry+"),
               channels = c("YG_610/20-A", "B_530/30-A"),
               overlay = c(Unst),
               density_fill_alpha=0.5,
               type = c("rectangle"))
cyto_gate_edit(gs[c(20,22)], parent = "Live cells",
               alias = c("mCherry+GFP+"),
               channels = c("YG_610/20-A", "B_530/30-A"),
               overlay = c(DT),
               density_fill_alpha=0.5,
...
cyto_gate_remove(gs, parent = "mCherry+",
                 ...
                 density_fill_alpha=0.5,
                 type = "threshold")
cyto_details(gs)$mutant[cyto_details(gs)$mutant=="NA"]<-NA
cyto_details(gs)$HK<-paste0(cyto_details(gs)$enzyme,cyto_details(gs)$mutant)
...
cyto_plot(gs,alias="",parent="Live cells",channels = c("YG_610/20-A", "B_530/30-A"), group_by = "HK")
...
cyto_plot(gs,
          ...
          density_stack = 1.3, popup = TRUE)
dev.off()
cyto_plot_save("../figs/mCherry+plot.png", width =15)
cyto_plot(gs[rev(plotorder)],
          parent = "mCherry+",
          alias = "",
          channels = c("B_530/30-A"),
...
cyto_gate_draw(gs, parent = "mCherry+",
               alias = c("GFP+"),
               channels = c("B_530/30-A"),
               overlay = c(Unst, DT),
               density_fill_alpha=0.5,
