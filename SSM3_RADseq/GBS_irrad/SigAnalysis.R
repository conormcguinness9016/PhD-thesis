...
SSM3genoms$sample<-str_extract(SSM3genoms$sample, "\\w{0,}(?=\\.trimmed)")
...
SSM3genoms$group<-NA
SSM3genoms$sample
SSM3genoms$group[str_detect(SSM3genoms$sample, "^0")]<-"0"
SSM3genoms$group[str_detect(SSM3genoms$sample, "^5")]<-"5"
SSM3genoms$group[str_detect(SSM3genoms$sample, "^10")]<-"10"
SSM3genoms$clone<-"mono"
SSM3genoms$clone[str_detect(SSM3genoms$sample, "_D")]<-"mixed"
SSM3genoms_ss<-subset(SSM3genoms, cov >19 & sample != "10P3.trimmed")
...
SSM3genomhets.gr<-subset(SSM3genoms.gr, VAF >0.35 & VAF<0.65)
...
#depthinf<-subset(depthinf, sample != "10P3")
...
depthinf
depthinf$group[str_detect(depthinf$sample, "^0")]<-"0"
depthinf$group[str_detect(depthinf$sample, "^5")]<-"5"
depthinf$group[str_detect(depthinf$sample, "^10")]<-"10"
depthinf$mutrate
depthinf
png("Rplots/mutratesallSNVs.png", width = 1500, height = 1000)
ggplot(depthinf, aes(sample, log(mutrate)))  +
...
dev.off()
plot(depthinf$greater20, depthinf$nSNVs)
